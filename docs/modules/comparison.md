...
- Delivery-method selector (single-select: All / Bank transfer / Cash pickup / Mobile wallet); changing it re-fetches with `method=` ✅ (web). When a specific method is selected the list shows **only providers that offer it**. A row passes the filter iff `quote.deliveryMethod === selectedMethod` **OR** `PROVIDERS[providerID].deliveryMethods?.includes(selectedMethod)` (editorial capability). Rows shown via the editorial branch (quote method ≠ selected) carry a small chip: "bank-transfer price" with tooltip. Western Union + TransferGo re-price per method (live quotes), so they always match. Best deal and average are computed over the method-matching set. See [exchange-rate](../services/exchange-rate.md#delivery-method-support-per-source). Multi-select "All toggles the rest off" simplified to single-select on web.
