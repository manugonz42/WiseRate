...
- **Registry:** `web/lib/data/corridors.ts` — `{ slug, from, to, fromLabel, toLabel, defaultAmount, title, metaDescription, intro, faq, hasCompareCTA }`. Adding a corridor = one registry entry; sitemap and static params derive from it.
...
- **CTA:** `/compare` only on corridors Compare actually supports — registry flag `hasCompareCTA` (true for `eur-to-php` only today, since Compare is EUR→PHP-only); other corridors' utility is the snapshot table itself + provider links.
...
- [x] Corridors enabled beyond EUR→PHP pass the ≥3-provider harness

## Corridor status

| Corridor | Slug | Status |
|---|---|---|
| EUR→PHP | `eur-to-php` | ✅ live (T13) |
| GBP→PHP | `gbp-to-php` | ✅ live — harness PASS, 13 providers (2026-07-03) |
| USD→PHP | `usd-to-php` | ✅ live — harness PASS, 9 providers (2026-07-03) |
| CAD→PHP | `cad-to-php` | ✅ live — harness PASS, 6 providers (2026-07-03) |
| AUD→PHP | `aud-to-php` | ✅ live — harness PASS, 10 providers (2026-07-03) |
