...
- [x] Same 3 Supabase env vars in Vercel (Production + Preview) — set 2026-07-17 via CLI
- [x] All 3 migrations applied 2026-07-17 via Management API (`20260717120000_accounts_foundation`, `20260717130000_referral_conversions`, `20260717140000_delete_account_fks` — the last fixes delete-account FK behavior). Verified e2e against the live project: RLS isolation, `referral_code` update rejected, full signup→attribution→click→conversion→reward→stats→delete-account chain green (22 checks)
- [x] `ADMIN_INGEST_TOKEN` env var (T37, `/api/admin/conversions`) — generated + set in `web/.env.local` and Vercel (Production + Preview) 2026-07-17; unset = route always 401s
